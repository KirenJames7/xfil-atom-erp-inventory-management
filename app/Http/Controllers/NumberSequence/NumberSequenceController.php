...
class NumberSequenceController extends Controller
{
    ...
    #multiple possible inputs FA category, document name & version
    public function generateNewNumber($id) {
        return $this->setNumberPrefix($id, $this->setNumberLength($id, $this->checkLastNumber($id) + 1));
    }
    
    public function generateNewDocumentNumber($document, $year, $company) {
